...
На вход программе подаются строка текста, содержащая символы, разделенные символом 
пробела, и число n на отдельной строке.
...
'''
def chunked():
    # Вводим символы через пробел
    symbols = input().split()
    # Вводим число для группировки символов в списке
    n = int(input())
    
    # Вводим переменную для списка чанков
    chunking_list = []
    
    # Проходим циклом по символам
    for i in range(0, len(symbols), n):
        # Создаем подсписок длиной n
        chunk = symbols[i:i + n]
        # Добавляем подсписок в список чанков
        chunking_list.append(chunk)
